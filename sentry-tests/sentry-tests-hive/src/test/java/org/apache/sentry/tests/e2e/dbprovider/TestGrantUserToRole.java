...
public class TestGrantUserToRole extends AbstractTestWithStaticConfiguration {
  ...
  private static String USER1 = "testGrantUserToRole_u1";
  private static String USER2 = "testGrantUserToRole_u2";
  private static String USER3 = "testGrantUserToRole_u3";
  private static String USER4 = "testGrantUserToRole_u4";
  private static String GROUP1 = "testGrantUserToRole_g1";
  private static String GROUP2 = "testGrantUserToRole_g2";
  private static String GROUP3 = "testGrantUserToRole_g3";
  ...
  private void prepareTestData() throws Exception {
    ...
    // user2->r3
    // user4->r3
    statement.execute("GRANT ROLE " + ROLENAME1 + " TO GROUP " + GROUP1);
    statement.execute("GRANT ROLE " + ROLENAME2 + " TO GROUP " + GROUP2);
    statement.execute("GRANT ROLE " + ROLENAME2 + " TO GROUP " + GROUP3);
    statement.execute("GRANT ROLE " + ROLENAME3 + " TO USER " + USER2);
    statement.execute("GRANT ROLE " + ROLENAME3 + " TO USER " + USER4);
    statement.close();
    connection.close();
  }
  ...
  public void testAddDeleteRolesForUser() throws Exception {
    ...
    resultSet = statement.executeQuery("SHOW ROLE GRANT USER " + USER3);
    verifyResultRoles(resultSet, Sets.newHashSet(ROLENAME1.toLowerCase(), ROLENAME2.toLowerCase()));

    // user4 get the role2 for group2 and group3, role3 for user4
    resultSet = statement.executeQuery("SHOW ROLE GRANT USER " + USER4);
    ...
    resultSet = statement.executeQuery("SHOW ROLE GRANT USER " + USER1);
    verifyResultRoles(resultSet, Sets.newHashSet(ROLENAME1.toLowerCase()));
    ...
    statement.execute("REVOKE ROLE " + ROLENAME3 + " FROM USER " + USER2);
    statement.execute("REVOKE ROLE " + ROLENAME3 + " FROM USER " + USER4);
  ...
  public void testAuthorizationForUsersWithRoles() throws Exception {
    ...
    // user3 can access the t1,t2
    connection = context.createConnection(USER3);
    statement = context.createStatement(connection);
    statement.execute("select c1 from t1");
    statement.execute("select c1 from t2");
    try {
    ...
    connection = context.createConnection(USER4);
    statement = context.createStatement(connection);
    try {
      statement.execute("select c1 from t1");
  ...
  private void verifyResultRoles(ResultSet resultSet, Set<String> exceptedRoles) throws Exception {
    ...
    while (resultSet.next()) {
      ...
      LOGGER.info("colinaabb:aaa:" + tempRole);
